...
def _rows(text, empty_text="Not specified"):
    ...
    return "".join(
        f'<div class="item-row"><span>{escape(item)}</span></div>'
        for item in items
    )
...
<div style="border:1px solid #e5e7eb;border-radius:10px;padding:6px 15px;">{_rows(configuration)}</div>
</td>
</tr>
<tr>
<td style="padding:0 32px 26px;">
<div style="font-size:17px;font-weight:700;margin-bottom:10px;">Accessories</div>
<div style="border:1px solid #e5e7eb;border-radius:10px;padding:6px 15px;">{_rows(accessories, "No accessories selected")}</div>
</td>
</tr>
<tr>
